...
document
  .getElementById("ButtonDepositar")
  .addEventListener("click", function () {
    ...
    if (errors2) {
      ...
      let saldoActualAntesDeDeposito =
        parseFloat(datosDeposito.SaldoInicial) + parseFloat(deposito_nuevo);
      // actulizamos el saldo en la sesion
      datosDeposito.SaldoInicial = saldoActualAntesDeDeposito;

      datosDeposito.transacciones.push({
        concepto: "Depósito en efectivo", // o cualquier otro concepto 
        fecha: new Date().toISOString().split("T")[0], // Fecha actual en formato YYYY-MM-DD
        monto: parseFloat(deposito_nuevo),
        tipo: "deposito",
      });
      ...
      procesarTransaccionesParaGraficos(datosDeposito.transacciones);
      calcularPromedioRetiros(datosDeposito.transacciones);
      localStorage.setItem("session_user", JSON.stringify(datosDeposito));
      location.reload();
